  ...
  <div class="course">
      ...
      <el-table-column fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <el-button @click="handleFile(scope.$index, scope.row)" type="text" size="small">归档</el-button>
          <el-button @click="handleToUnfinish(scope.$index, scope.row)" type="text" size="small">撤回</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    getData() {
      axios("get", "home/teacherInformation/showTeacherInfo", {})
        .then(res => {
          ...
          axios("get", "home/invigilateExam/showInvigilateExamTable:finish", {})
          .then(res => {
            console.log(res.data);
    ...
    handleFile(index, row){
      ...
      axios("post", "home/invigilateExam/showInvigilateExamTable:file", {invigilateObject: JSON.stringify(row)})
        .then(res => {
          console.log(res.data);    
        })
        .catch(err => {
    ...
    handleToUnfinish(index, row){
       console.log("handleToUnfinish");
      axios("post", "home/invigilateExam/showInvigilateExamTable/toUnfinish", {invigilateObject: JSON.stringify(row)})
        .then(res => {
          console.log(res.data);    
        })
        .catch(err => {
